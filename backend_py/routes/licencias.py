...
async def create(body: dict, current_user: dict = Depends(get_current_user)):
    ...
    await log_action(
        user_id=user_id,
        accion="Creó licencia",
        modulo="Licencias",
        entidad_id=nuevo["id"],
        detalle=f"Nombre: {body['nombre']}, Cantidad total: {body.get('cantidad_total', 1)}"
    )
...
async def delete(id: str, current_user: dict = Depends(get_current_user)):
    licencia = await LicenciaModel.find_by_id(id)
    if not licencia:
        raise HTTPException(status_code=404, detail="Licencia no encontrada.")
    ...
    await log_action(
        user_id=user_id,
        accion="Eliminó licencia",
        modulo="Licencias",
        entidad_id=id,
        detalle=f"Nombre: {licencia.get('nombre', 'N/A')}"
    )
...
async def crear_asignacion(licencia_id: str, body: dict, current_user: dict = Depends(get_current_user)):
    ...
    await log_action(
        user_id=user_id,
        accion="Asignó licencia",
        modulo="Licencias",
        entidad_id=nueva["id"],
        detalle=f"Licencia ID: {licencia_id}, Serial: {body.get('serial', 'N/A')}, Usuario: {body.get('usuario', 'N/A')}, Equipo: {body.get('equipo_id', 'N/A')}"
    )

    return serialize({"data": nueva, "message": "Licencia asignada exitosamente."})

@router.put("/asignaciones/{id}")
async def actualizar_asignacion(id: str, body: dict, current_user: dict = Depends(get_current_user)):
    ...
    await log_action(
        user_id=user_id,
        accion="Actualizó asignación de licencia",
        modulo="Licencias",
        entidad_id=id,
        detalle=f"Campos actualizados: {list(body.keys())}"
    )
...
async def liberar_asignacion(id: str, current_user: dict = Depends(get_current_user)):
    if not await LicenciaAsignadaModel.find_by_id(id):
        raise HTTPException(status_code=404, detail="Asignación no encontrada.")
    ...
    await log_action(
        user_id=user_id,
        accion="Liberó licencia",
        modulo="Licencias",
        entidad_id=id,
        detalle=f"Asignación liberada (ID: {id})"
    )

    return serialize({"data": actualizado, "message": "Licencia liberada."})

@router.delete("/asignaciones/{id}")
async def eliminar_asignacion(id: str, current_user: dict = Depends(get_current_user)):
    if not await LicenciaAsignadaModel.find_by_id(id):
        raise HTTPException(status_code=404, detail="Asignación no encontrada.")
    ...
    await log_action(
        user_id=user_id,
        accion="Eliminó asignación de licencia",
        modulo="Licencias",
        entidad_id=id,
        detalle=f"Asignación ID: {id}"
    )
