    ...
    *,
    ...
    if es_portatil:
        ...
        for i in paginas_a_conservar:
            if i < len(tpl.pages):
                temp_writer.add_page(tpl.pages[i])
        print(f"[DEBUG] Conservando páginas: {paginas_a_conservar} de {len(tpl.pages)}")
    
    temp_bytes = io.BytesIO()
    temp_writer.write(temp_bytes)
    temp_bytes.seek(0)
    tpl = PdfReader(temp_bytes)
    print(f"[DEBUG] Nuevas páginas: {len(tpl.pages)}")
    # ============================================================
